...
class ShopTools
{
    ...
    public static function getSession(?HTTPRequest $httpRequest = null): Session
    {
        ...
        if (($controller = Controller::curr()) && ($httpRequest = $controller->getRequest())) {
            if ($session = $httpRequest->getSession()) {
                return $session;
            }
        }

        try {
            $httpRequest = Injector::inst()->get(HTTPRequest::class);
            if ($httpRequest && ($session = $httpRequest->getSession())) {
                return $session;
            }
        ...
        // Last resort in SS6: get the session singleton via Injector rather than
        // returning new Session([]) which creates an orphaned object not wired to any request.
